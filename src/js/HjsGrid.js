...
class HjsGrid {
    ...
    #renderLeftRow = (rowIdx) => {
        ...
        for(let idx=0;idx<leftBodySelectArray.length;idx++){
            let sa = leftBodySelectArray[idx];

            if(sa.startRowIndex <= rowIdx && rowIdx <= sa.endRowIndex){
                selectYn = true;
                break;
            }
        }
        ...
        if(selectYn) trEl.querySelectorAll("td").forEach(td => td.classList.add("select"))
    }
    ...
    #createCell = (rowIdx,colIdx) => {
        ...
        for(let idx=0;idx<bodySelectArray.length;idx++){
            let sa = bodySelectArray[idx];

            if(sa.startRowIndex <= rowIdx && rowIdx <= sa.endRowIndex && sa.startColIndex <= colIdx && colIdx <= sa.endColIndex){
                selectYn = true;
                break;
            }
        }
        ...
        if(selectYn) tdEl.classList.add("select")
        ...
        if(newSelectArray.length > 0){
            ...
            if(!this.#isUN(curInfo)){
                ...
                if((top+height<=EL_HEIGHT && !rowspanYn) || (rowspanYn)){
                    ...
                    let leftSelectYn = this.#utils.get("select").get("leftSelectYn");
                    let curDivwidth = leftSelectYn?this.#columnsOption.get("columnsTotalWidth"):this.#columns[curInfo.colIdx].width
                    let curEditorWidth = this.#columns[curInfo.colIdx].width
                    
                    // left+width <= EL_WIDTH
                    if(true){
                        curDivEl.style.left = left + "px";
                        curDivEl.style.width = curDivwidth + "px";
                        curDivEl.style.opacity = "1";
                        if(leftSelectYn) curDivEl.style.borderLeft = "0px";
                        else curDivEl.style.borderLeft = ""
                        ...
                        if(!this.#isUN(this.el.get("middleBodySelectCurrentEditor")) && this.el.get("middleBodySelectCurrentEditor")?.style?.opacity === "0"){
                            ...
                            this.el.get("middleBodySelectCurrentEditor").style.width = curEditorWidth + "px"
                        }
    ...
    #getRowspanInfo = (rowIdx, colName) => {
        ...
        return [startIndex,rowspanNum,Math.min(Math.max(startIndex,startIndex+rowspanNum-1),this.#data.get("showData").length-1)];
    }
    ...
    #gridCellMouseDown = (e) => {
        ...
        if(!(e.target.classList.contains("hjs-grid-editor") && e.target.style.opacity !== "0")){
            ...
            this.#utils.get("select").set("bodySelectStartRowspanInfo",{
                rowspanSet : new Set()
                , startRowIndex : rowIdx
                , endRowIndex : rowIdx
                , startColIndex : colIdx
    ...
    #gridCellMouseMove = (e) => {
        ...
        if(this.#utils.get("select").get("bodySelectFlag") === true){
            ...
            let seRow = startJson.rowIdx;

            let tsRow = rowIdx;
            let teRow = rowIdx;

            if(rowspanYn){
                selectRowspanInfo.rowspanSet.add(colIdx)
            }
            ...
            selectJson.startRowIndex = Math.min(ssRow,tsRow);
            selectJson.endRowIndex = Math.max(seRow,teRow);
    ...
    #editorKeyDown = (e,rowId,colNm,editorEl)=> {
        ...
        if(e.keyCode === 13){   // enter
            ...
            if(!this.#utils.get("select").get("leftSelectYn")) this.#f2KeyFunction();
        }else if(e.keyCode === 46){ // delete
            this.#deleteKeyFunction(rowIdx,colIdx);
        }else{
            if(e.isComposing) return;
    ...
    #arrowLeftShiftCtrlKeyFunction = e => {
        ...
        while(startFlag){
            for(let colIdx=this.#utils.get("select").get("bodySelectArray")[sa.index].startColIndex;colIdx<=this.#utils.get("select").get("bodySelectArray")[sa.index].endColIndex;colIdx++){
                ...
                let minRowspanYn = this.#getRowspanYn(colIdx);
                let minRowspanInfo = this.#getRowspanInfo(startRowIndex,colIdx);
    
                if(minRowspanYn &&!this.#isUN(minRowspanInfo) && startRowIndex > minRowspanInfo[0]){
                    startRowIndex = minRowspanInfo[0];
                    break;
        ...
        this.#utils.get("select").get("bodySelectArray")[sa.index].startRowIndex = startRowIndex

        let endRowIndex = this.#utils.get("select").get("bodySelectArray")[sa.index].endRowIndex;
        ...
        while(endFlag){
            for(let colIdx=this.#utils.get("select").get("bodySelectArray")[sa.index].startColIndex;colIdx<=this.#utils.get("select").get("bodySelectArray")[sa.index].endColIndex;colIdx++){
                ...
                let maxRowspanYn = this.#getRowspanYn(colIdx);
                let maxRowspanInfo = this.#getRowspanInfo(endRowIndex,colIdx);
    
                if(maxRowspanYn &&!this.#isUN(maxRowspanInfo) && endRowIndex < maxRowspanInfo[2]){
                    endRowIndex = maxRowspanInfo[2];
                    break;
                }else if(colIdx === this.#utils.get("select").get("bodySelectArray")[sa.index].endColIndex){
                    endFlag = false;
                }
    ...
    #arrowUpShiftKeyFunction = e => {
        ...
        while(flag){
            for(let colIdx=this.#utils.get("select").get("bodySelectArray")[sa.index].startColIndex;colIdx<=this.#utils.get("select").get("bodySelectArray")[sa.index].endColIndex;colIdx++){
                if(this.#columns[colIdx].hidden === true || this.#columns[colIdx].fixed === true) continue;
                let rowspanYn = this.#getRowspanYn(colIdx);
                let rowspanInfo = this.#getRowspanInfo(curInfo.startRowIndex,colIdx);
                
                if(rowspanYn && curInfo.startRowIndex > rowspanInfo[0]){
                    curInfo.startRowIndex = rowspanInfo[0]
                    break;
                }else if(colIdx === this.#utils.get("select").get("bodySelectArray")[sa.index].endColIndex){
                    flag = false;
                }
        ...
        while(flag){
            for(let colIdx=this.#utils.get("select").get("bodySelectArray")[sa.index].startColIndex;colIdx<=this.#utils.get("select").get("bodySelectArray")[sa.index].endColIndex;colIdx++){
                if(this.#columns[colIdx].hidden === true || this.#columns[colIdx].fixed === true) continue;
                let rowspanYn = this.#getRowspanYn(colIdx);
                let rowspanInfo = this.#getRowspanInfo(curInfo.endRowIndex,colIdx);
                
                if(rowspanYn && curInfo.endRowIndex < rowspanInfo[2]){
                    curInfo.endRowIndex = rowspanInfo[2]
                    break;
                }else if(colIdx === this.#utils.get("select").get("bodySelectArray")[sa.index].endColIndex){
                    flag = false;
                }
        ...
        if(sa.startRowIndex === curInfo.startRowIndex && sa.endRowIndex > curInfo.endRowIndex){
            ...
            this.#utils.get("select").get("bodySelectArray")[sa.index].startRowIndex = Math.min(endRowIndex,curInfo.startRowIndex)
        }
    ...
    #arrowDownShiftKeyFunction = e => {
        ...
        */
        ...
        /*if(rowspanYn && sa.startRowIndex === rowspanInfo[0] && sa.endRowIndex === rowspanInfo[2]){
            this.#utils.get("select").get("bodySelectArray")[sa.index].endRowIndex = Math.max(Math.min(rowspanInfo[2]+1,this.#data.get("showData").length-1),0)
        }else if(sa.endRowIndex === curInfo.rowIdx && curInfo.rowIdx !== sa.startRowIndex && !this.#isUN(sa.endRowIndex) & !this.#isUN(curInfo.rowIdx)){
            this.#utils.get("select").get("bodySelectArray")[sa.index].startRowIndex = Math.max(Math.min(this.#utils.get("select").get("bodySelectArray")[sa.index].startRowIndex+1,this.#data.get("showData").length-1),0)
        }else{
            this.#utils.get("select").get("bodySelectArray")[sa.index].endRowIndex = Math.max(Math.min(this.#utils.get("select").get("bodySelectArray")[sa.index].endRowIndex+1,this.#data.get("showData").length-1),0)
        }*/

        /*let minRowIndex = this.#utils.get("select").get("bodySelectArray")[sa.index].startRowIndex;
        let minFlag = true;

        while(minFlag){
            for(let colIdx=this.#utils.get("select").get("bodySelectArray")[sa.index].startColIndex+1;colIdx<=this.#utils.get("select").get("bodySelectArray")[sa.index].endColIndex;colIdx++){
                if(this.#columns[colIdx].hidden === true || this.#columns[colIdx].fixed === true) continue;
    
                let minRowspanYn = this.#getRowspanYn(colIdx);
                let minRowspanInfo = this.#getRowspanInfo(minRowIndex,colIdx);
    
                if(minRowspanYn &&!this.#isUN(minRowspanInfo) && minRowIndex > minRowspanInfo[0]){
                    minRowIndex = minRowspanInfo[0];
                    break;
                }else if(colIdx === this.#utils.get("select").get("bodySelectArray")[sa.index].endColIndex){
                    minFlag = false;
                }
        ...
        this.#utils.get("select").get("bodySelectArray")[sa.index].startRowIndex = minRowIndex
        */
